...
function pushBarShapes(shapes, rowNo, item, projectStart, totalWeeks, weekWidthsPx) {
  ...
  const color = item.kind === 'phase' ? colors.svg : colors.subSvg;
...
async function buildGanttWorkbook(input) {
  ...
  sheet.getCell(3, 4).value = '状态/完成度';
  sheet.getCell(3, 5).value = '负责人（甲方）';
  sheet.getCell(3, 6).value = '负责人（乙方）';
  ...
  styleCell(sheet.getCell(3, 4), { fill: 'FFF0F7FF', color: 'FF1E40AF', bold: true, size: 12, wrapText: true });
  styleCell(sheet.getCell(3, 5), { fill: 'FFEDF3FB', color: 'FF36506A', bold: true, size: 12, horizontal: 'left', wrapText: true });
  styleCell(sheet.getCell(3, 6), { fill: 'FFEDF3FB', color: 'FF36506A', bold: true, size: 12, horizontal: 'left', wrapText: true });
  ...
  for (let index = 0; index < project.rows.length; index += 1) {
    ...
    sheet.getCell(rowNo, 4).value = isPhase ? `${normalizeProgress(item.progress)}%` : normalizeTaskStatus(item.status);
    sheet.getCell(rowNo, 5).value = isPhase ? '' : (item.ownerClient || '');
    sheet.getCell(rowNo, 6).value = isPhase ? '' : (item.ownerVendor || '');
    ...
    styleCell(sheet.getCell(rowNo, 4), {
      fill: isPhase ? 'FFEAF3FF' : 'FFF6FAFF',
      color: isPhase ? 'FF1D4ED8' : 'FF334155',
      bold: isPhase,
      shrinkToFit: true,
    });
    styleCell(sheet.getCell(rowNo, 5), {
      fill: isPhase ? 'FFF4F8FD' : 'FFFAFCFF',
      color: 'FF475569',
      bold: isPhase,
      horizontal: 'left',
      shrinkToFit: true,
    });
    styleCell(sheet.getCell(rowNo, 6), {
      fill: isPhase ? 'FFF4F8FD' : 'FFFAFCFF',
      color: 'FF475569',
      bold: isPhase,
      horizontal: 'left',
...
function buildGanttSvg(project) {
  ...
  const padding = 10;
  const timelineLeft = padding + leftA + leftB + leftC + leftD + leftE + leftF;
  const width = padding * 2 + leftA + leftB + leftC + leftD + leftE + leftF + project.totalWeeks * weekWidth + 20;
  ...
  const headCells = [
    ...
    { x: padding + leftA + leftB + leftC, w: leftD, text: '状态/完成度', fill: '#F0F7FF', color: '#1E40AF', align: 'middle' },
    { x: padding + leftA + leftB + leftC + leftD, w: leftE, text: '负责人（甲方）', fill: '#EDF3FB', color: '#36506A', align: 'left' },
    { x: padding + leftA + leftB + leftC + leftD + leftE, w: leftF, text: '负责人（乙方）', fill: '#EDF3FB', color: '#36506A', align: 'left' },
  ];
  ...
  for (let index = 0; index < project.rows.length; index += 1) {
    ...
    parts.push(`<rect x="${padding + leftA + leftB + leftC}" y="${y}" width="${leftD}" height="${rowHeight}" fill="${isPhase ? '#EAF3FF' : '#F6FAFF'}" stroke="#DCE5F2"/>`);
    parts.push(`<rect x="${padding + leftA + leftB + leftC + leftD}" y="${y}" width="${leftE}" height="${rowHeight}" fill="${isPhase ? '#F4F8FD' : '#FAFCFF'}" stroke="#DCE5F2"/>`);
    parts.push(`<rect x="${padding + leftA + leftB + leftC + leftD + leftE}" y="${y}" width="${leftF}" height="${rowHeight}" fill="${isPhase ? '#F4F8FD' : '#FAFCFF'}" stroke="#DCE5F2"/>`);
    ...
    parts.push(`<text class="small" x="${padding + leftA + leftB + leftC + leftD / 2}" y="${y + 22}" fill="${isPhase ? '#1D4ED8' : '#334155'}" text-anchor="middle" font-weight="${isPhase ? '700' : '500'}">${escapeXml(isPhase ? `${normalizeProgress(item.progress)}%` : normalizeTaskStatus(item.status))}</text>`);
    parts.push(`<text class="small" x="${padding + leftA + leftB + leftC + leftD + 10}" y="${y + 22}" fill="#475569">${escapeXml(isPhase ? '' : (item.ownerClient || ''))}</text>`);
    parts.push(`<text class="small" x="${padding + leftA + leftB + leftC + leftD + leftE + 10}" y="${y + 22}" fill="#475569">${escapeXml(isPhase ? '' : (item.ownerVendor || ''))}</text>`);
