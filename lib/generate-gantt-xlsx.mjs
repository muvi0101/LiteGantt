...
function weekWidth(weekIndex) {
  return weekIndex === 1 ? WEEK1_WIDTH_PX : WEEK_WIDTH_PX;
}
...
function dayPxAtOffset(offset) {
  const weekIndex = Math.floor(offset / 7) + 1;
  return weekWidth(weekIndex) / 7;
}

function anchorByDayOffset(offset) {
  const weekIndex = Math.floor(offset / 7) + 1;
  const dayInWeek = offset % 7;
  return {
    col: (WEEK_START_COL - 1) + (weekIndex - 1),
    colOffsetPx: Math.round(dayInWeek * dayPxAtOffset(offset)) + 2,
  };
}

function timelinePxByDayOffset(offset) {
  const weekIndex = Math.floor(offset / 7) + 1;
  const dayInWeek = offset % 7;
  let px = 2;
  for (let index = 1; index < weekIndex; index += 1) px += weekWidth(index);
  return Math.round(px + dayInWeek * dayPxAtOffset(offset));
}

function anchorByTimelinePx(timelinePx, totalWeeks = MAX_WEEKS) {
  let remainingPx = Math.max(0, Math.round(timelinePx));
  for (let weekIndex = 1; weekIndex <= totalWeeks; weekIndex += 1) {
    const widthPx = weekWidth(weekIndex);
  ...
  return {
    ...
    colOffsetPx: Math.max(0, weekWidth(totalWeeks) - 1),
  };
}

function widthPxForDays(startOffset, durationDays) {
  let px = 0;
  for (let index = 0; index < durationDays; index += 1) {
    px += dayPxAtOffset(startOffset + index);
  }
...
function pushBarShapes(shapes, rowNo, item, projectStart, totalWeeks) {
  const durationDays = daysInclusive(item.start, item.end);
  const startOffset = dayOffset(item.start, projectStart);
  const { col, colOffsetPx } = anchorByDayOffset(startOffset);
  const timelineLeftPx = timelinePxByDayOffset(startOffset);
  const exactWidthPx = widthPxForDays(startOffset, durationDays);
  ...
  return { capsuleWidthPx, timelineLeftPx, totalWeeks };
}
...
function pushMilestoneShapes(shapes, rowNo, item, barAnchor) {
  ...
  const starAnchor = anchorByTimelinePx(starTimelinePx, barAnchor.totalWeeks);
  const textAnchor = anchorByTimelinePx(textTimelinePx, barAnchor.totalWeeks);
...
async function buildGanttWorkbook(input) {
  ...
  sheet.columns = [
    ...
    { width: 24 },
    { width: 14 },
    ...Array.from({ length: project.totalWeeks }, (_, index) => ({ width: index === 0 ? 12 : 9 })),
    { width: 3 },
  ];
  ...
  for (let index = 0; index < project.rows.length; index += 1) {
    ...
    const barAnchor = pushBarShapes(drawingShapes, rowNo, item, project.projectStart, project.totalWeeks);
