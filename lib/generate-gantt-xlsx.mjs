...
async function buildGanttWorkbook(input) {
  ...
  sheet.getCell(3, 4).value = '负责人（甲方）';
  sheet.getCell(3, 5).value = '负责人（乙方）';
  ...
  styleCell(sheet.getCell(3, 4), { fill: 'FFEDF3FB', color: 'FF36506A', bold: true, size: 12, horizontal: 'left', wrapText: true });
  ...
  for (let index = 0; index < project.rows.length; index += 1) {
    ...
    sheet.getCell(rowNo, 4).value = isPhase ? '' : (item.ownerClient || '');
    sheet.getCell(rowNo, 5).value = isPhase ? '' : (item.ownerVendor || '');
  ...
  for (let rowNo = FIRST_DATA_ROW; rowNo <= sheet.rowCount; rowNo += 1) {
    ...
    if (!isTask) {
      ...
      currentPhase.tasks.push({
        ...
        ownerClient: String(sheet.getCell(rowNo, 4).value || '').trim(),
        ownerVendor: String(sheet.getCell(rowNo, 5).value || '').trim(),
...
function buildGanttSvg(project) {
  ...
  const padding = 10;
  const timelineLeft = padding + leftA + leftB + leftC + leftD + leftE;
  const width = padding * 2 + leftA + leftB + leftC + leftD + leftE + project.totalWeeks * weekWidth + 20;
  ...
  const headCells = [
    ...
    { x: padding + leftA + leftB + leftC, w: leftD, text: '负责人（甲方）', fill: '#EDF3FB', color: '#36506A', align: 'left' },
    { x: padding + leftA + leftB + leftC + leftD, w: leftE, text: '负责人（乙方）', fill: '#EDF3FB', color: '#36506A', align: 'left' },
  ];
  ...
  for (let index = 0; index < project.rows.length; index += 1) {
    ...
    parts.push(`<rect x="${padding + leftA + leftB + leftC}" y="${y}" width="${leftD}" height="${rowHeight}" fill="${isPhase ? '#F4F8FD' : '#FAFCFF'}" stroke="#DCE5F2"/>`);
    ...
    parts.push(`<text class="small" x="${padding + leftA + leftB + leftC + 10}" y="${y + 22}" fill="#475569">${escapeXml(isPhase ? '' : (item.ownerClient || ''))}</text>`);
    parts.push(`<text class="small" x="${padding + leftA + leftB + leftC + leftD + 10}" y="${y + 22}" fill="#475569">${escapeXml(isPhase ? '' : (item.ownerVendor || ''))}</text>`);
