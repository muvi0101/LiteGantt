...
function getMonthBandsFromColumns(columns) {
  const monthBands = [];
  let activeBand = null;
  for (const column of columns) {
    const label = monthLabel(column.start);
    if (!activeBand || activeBand.label !== label) {
      ...
      activeBand.toIndex = column.index;
    }
  }
  return monthBands;
}

function getYearBandsFromColumns(columns) {
  const yearBands = [];
  let activeBand = null;
  for (const column of columns) {
    const label = `${column.start.getUTCFullYear()}年`;
    if (!activeBand || activeBand.label !== label) {
      activeBand = {
        label,
        fromIndex: column.index,
...
function applyBandLabelWidths(columns, bands) {
  const adjustedColumns = columns.map((column) => ({ ...column }));
  bands.forEach((band) => {
    const requiredWidth = estimateHeaderWidthPx(band.label) + MONTH_BAND_LABEL_PADDING_PX;
    const spanColumns = adjustedColumns.slice(band.fromIndex - 1, band.toIndex);
...
function buildDayColumns(project, options = {}) {
  const totalDays = options.includeMilestoneLabelSpace === false
    ? project.baseDays
    : project.totalDays;
  const columns = Array.from({ length: totalDays }, (_, index) => {
    const date = addDays(project.projectStart, index);
    return {
      index: index + 1,
      start: date,
      end: date,
      label: String(date.getUTCDate()),
      subLabel: ['周日', '周一', '周二', '周三', '周四', '周五', '周六'][date.getUTCDay()],
      isWeekend: date.getUTCDay() === 0 || date.getUTCDay() === 6,
      widthPx: DAY_WIDTH_PX,
      excelWidth: excelColumnWidthFromPx(DAY_WIDTH_PX),
    };
  });
  return applyBandLabelWidths(columns, getMonthBandsFromColumns(columns));
}

function buildMonthColumns(project) {
  const columns = [];
  let cursor = startOfMonth(project.projectStart);
  const endMonth = startOfMonth(project.projectEnd);
  while (cursor <= endMonth && columns.length < MAX_MONTHS) {
    ...
    columns.push({
      ...
      subLabel: `${fmtMd(cursor)}-${fmtMd(end)}`,
      widthPx: MONTH_WIDTH_PX,
      excelWidth: excelColumnWidthFromPx(MONTH_WIDTH_PX),
    });
    cursor = addMonths(cursor, 1);
  }
  while (columns.length < 4) {
    const last = columns[columns.length - 1];
    const nextStart = last ? addMonths(last.start, 1) : startOfMonth(project.projectStart);
    ...
    columns.push({
      ...
      subLabel: `${fmtMd(nextStart)}-${fmtMd(nextEnd)}`,
      widthPx: MONTH_WIDTH_PX,
      excelWidth: excelColumnWidthFromPx(MONTH_WIDTH_PX),
    });
  }
...
function buildTimelineColumns(project, unit = project.timelineUnit, options = {}) {
  const timelineUnit = normalizeTimelineUnit(unit);
  const columns = timelineUnit === 'day'
    ? buildDayColumns(project, options)
    : timelineUnit === 'month'
      ? buildMonthColumns(project)
      : buildWeekColumns(project);
  return {
    unit: timelineUnit,
    label: TIMELINE_UNIT_LABELS[timelineUnit] || TIMELINE_UNIT_LABELS.week,
    columns,
    widthsPx: columns.map((column) => column.widthPx),
...
function estimateTextWidthPx(text) {
  ...
  for (const ch of String(text)) {
    const code = ch.codePointAt(0);
    if (ch === ' ') width += 4;
    else if (code <= 0x007f) width += /[A-Za-z0-9]/.test(ch) ? 7 : 5;
    else width += 13;
  }
...
function estimateMilestoneLabelWidthPx(task) {
  const label = `${task.name} (${fmtMd(task.end)})`;
...
function getExtraWeeksForMilestones(projectStart, phases, baseWeeks) {
  ...
  phases.forEach((phase) => {
    phase.tasks.forEach((task) => {
      ...
      const markerGapPx = Number.isFinite(task.markerGapPx) ? task.markerGapPx : MILESTONE_BAR_TO_STAR_GAP_PX;
...
function getExtraDaysForMilestones(projectStart, phases, baseDays) {
  const baseTrackWidth = baseDays * DAY_WIDTH_PX;
  let extraDays = 0;
  ...
  phases.forEach((phase) => {
    phase.tasks.forEach((task) => {
      if (!task.milestone) return;
      const markerGapPx = Number.isFinite(task.markerGapPx) ? task.markerGapPx : MILESTONE_BAR_TO_STAR_GAP_PX;
      const markerRightPx = (dayOffset(task.end, projectStart) + 1) * DAY_WIDTH_PX
        + markerGapPx
        + MILESTONE_STAR_SIZE_PX
        + MILESTONE_STAR_TO_TEXT_GAP_PX
        + estimateMilestoneLabelWidthPx(task);
      const overflowPx = Math.max(0, markerRightPx - baseTrackWidth);
      if (overflowPx > 0) {
        extraDays = Math.max(extraDays, Math.ceil(overflowPx / DAY_WIDTH_PX));
      }
    });
  });
...
function normalizeProject(input) {
  ...
  const timelineUnit = normalizeTimelineUnit(input?.timelineUnit);
  ...
  const baseDays = Math.max(1, daysInclusive(projectStart, projectEnd));
  const extraDays = getExtraDaysForMilestones(projectStart, normalized, baseDays);
  const totalDays = Math.max(1, Math.min(MAX_WEEKS * 7, baseDays + extraDays));
  ...
  return { title, timelineUnit, phases: normalized, rows, projectStart, projectEnd, baseDays, totalDays, totalWeeks };
}
...
function serializeProject(project) {
  return {
    title: project.title,
    timelineUnit: normalizeTimelineUnit(project.timelineUnit),
...
function pushBarShapes(shapes, rowNo, item, timeline) {
  const durationDays = daysInclusive(item.start, item.end);
  const pointPx = item.milestone ? timelinePxByDate(item.end, timeline.columns, true) : null;
  const drawAsMilestonePoint = item.kind === 'sub' && item.milestone && durationDays === 1;
  const timelineRightPx = drawAsMilestonePoint
    ? pointPx
    : timelinePxByDate(item.end, timeline.columns, true);
  const timelineLeftPx = drawAsMilestonePoint
    ? timelineRightPx - MILESTONE_MARKER_WIDTH_PX
    : timelinePxByDate(item.start, timeline.columns, false);
  const fromAnchor = anchorByTimelinePx(timelineLeftPx, timeline.columns.length, timeline.widthsPx);
  const toAnchor = anchorByTimelinePx(timelineRightPx, timeline.columns.length, timeline.widthsPx);
  ...
  return {
    ...
    totalWeeks: timeline.columns.length,
    weekWidthsPx: timeline.widthsPx,
  };
}

function pushMilestoneShapes(shapes, rowNo, item, barAnchor) {
  if (!item.milestone) return;
  const markerGapPx = Number.isFinite(item.markerGapPx) ? item.markerGapPx : MILESTONE_BAR_TO_STAR_GAP_PX;
  const label = `${item.name} (${fmtMd(item.end)})`;
  const starTimelinePx = barAnchor.milestonePointPx + markerGapPx;
...
function timelinePxByDate(date, columns, includeEnd = false) {
  if (!(date instanceof Date) || !Array.isArray(columns) || !columns.length) return 0;
  if (date < columns[0].start) return 0;

  let leftPx = 0;
  for (const column of columns) {
    const widthPx = column.widthPx || WEEK_WIDTH_PX;
    if (date <= column.end) {
      const columnDays = Math.max(1, daysInclusive(column.start, column.end));
      const dayInColumn = clamp(
        Math.round((date - column.start) / DAY) + (includeEnd ? 1 : 0),
        0,
        columnDays,
      );
      return leftPx + (dayInColumn * widthPx) / columnDays;
    }
    leftPx += widthPx;
  }
...
function formatGanttDuration(item, unit = 'week') {
  const durationDays = daysInclusive(item.start, item.end);
  const normalizedUnit = normalizeTimelineUnit(unit);
  if (item.kind === 'phase' && normalizedUnit === 'month') {
    return `${Math.max(1, Math.ceil(durationDays / 30))}月（${fmtMd(item.start)}-${fmtMd(item.end)}）`;
  }
  if (item.kind === 'phase' && normalizedUnit === 'day') {
    return `${durationDays}天（${fmtMd(item.start)}-${fmtMd(item.end)}）`;
  }
  if (item.kind !== 'phase') {
    return `${workingDaysInclusive(item.start, item.end)}个工作日（${fmtMd(item.start)}-${fmtMd(item.end)}）`;
  }
  return `${Math.ceil(durationDays / 7)}周（${fmtMd(item.start)}-${fmtMd(item.end)}）`;
}
...
function mergeAndStyleTimelineBand(sheet, row, fromIndex, toIndex, value, options = {}) {
  const fromCol = WEEK_START_COL + fromIndex - 1;
  const toCol = WEEK_START_COL + toIndex - 1;
  if (fromCol !== toCol) sheet.mergeCells(row, fromCol, row, toCol);
  sheet.getCell(row, fromCol).value = value;
  styleCell(sheet.getCell(row, fromCol), options);
}

function writeTimelineHeaders(sheet, timeline) {
  if (timeline.unit === 'month') {
    for (const band of getYearBandsFromColumns(timeline.columns)) {
      mergeAndStyleTimelineBand(sheet, 3, band.fromIndex, band.toIndex, band.label, {
        fill: 'FFE6EBF3',
        bold: true,
        shrinkToFit: true,
      });
    ...
    for (const band of getMonthBandsFromColumns(timeline.columns)) {
      mergeAndStyleTimelineBand(sheet, 3, band.fromIndex, band.toIndex, band.label, {
        fill: 'FFE6EBF3',
        bold: true,
        shrinkToFit: true,
  ...
  for (const column of timeline.columns) {
    const col = WEEK_START_COL + column.index - 1;
    if (timeline.unit === 'week') {
      sheet.getCell(4, col).value = `W${column.index}`;
      sheet.getCell(5, col).value = column.label;
    } else if (timeline.unit === 'day') {
      sheet.getCell(4, col).value = column.label;
      sheet.getCell(5, col).value = column.subLabel;
    } else {
      sheet.getCell(4, col).value = column.label;
      sheet.getCell(5, col).value = column.subLabel;
    }

    const weekendFill = timeline.unit === 'day' && column.isWeekend ? 'FFFFF7ED' : 'FFEEF2F8';
    styleCell(sheet.getCell(4, col), { fill: weekendFill, bold: true, shrinkToFit: true });
    styleCell(sheet.getCell(5, col), {
      fill: timeline.unit === 'day' && column.isWeekend ? 'FFFFFBEB' : 'FFF5F8FC',
      color: timeline.unit === 'day' && column.isWeekend ? 'FFB45309' : 'FF64748B',
      size: 9,
      shrinkToFit: true,
    });
  }
...
async function buildGanttWorkbook(input) {
  ...
  const timeline = buildTimelineColumns(project);
  const drawingShapes = [];
  ...
  const sheet = workbook.addWorksheet(`${timeline.label}甘特图`, {
    views: [{ state: 'frozen', xSplit: 2, ySplit: 5 }],
  });
  const timelineEndCol = WEEK_START_COL + timeline.columns.length - 1;
  const lastCol = timelineEndCol + 1;
  ...
  sheet.columns = [
    ...
    ...timeline.columns.map((column) => ({ width: column.excelWidth })),
    { width: 3 },
  ];

  sheet.mergeCells(1, 1, 1, timelineEndCol);
  ...
  for (let index = 0; index < project.rows.length; index += 1) {
    ...
    for (const column of timeline.columns) {
      ...
      const weekendFill = timeline.unit === 'day' && column.isWeekend ? (isPhase ? 'FFFFF7ED' : 'FFFFFBEB') : background;
      styleCell(cell, { fill: weekendFill });
    }

    const barAnchor = pushBarShapes(drawingShapes, rowNo, item, timeline);
...
function buildGanttSvg(project) {
  ...
  const timeline = buildTimelineColumns(project);
  const width = padding * 2 + leftPhaseWidth + leftDurationWidth + timeline.widthPx;
  ...
  function columnLeftPx(index) {
    return timeline.widthsPx.slice(0, Math.max(0, index - 1)).reduce((sum, widthPx) => sum + widthPx, 0);
  }

  function columnSpanWidth(fromIndex, toIndex) {
    return timeline.widthsPx.slice(fromIndex - 1, toIndex).reduce((sum, widthPx) => sum + widthPx, 0);
  }

  function drawBand(band) {
    const x = timelineLeft + columnLeftPx(band.fromIndex);
    const w = columnSpanWidth(band.fromIndex, band.toIndex);
  ...
  const bands = timeline.unit === 'month'
    ? getYearBandsFromColumns(timeline.columns)
    : getMonthBandsFromColumns(timeline.columns);
  bands.forEach(drawBand);

  for (const column of timeline.columns) {
    const x = timelineLeft + columnLeftPx(column.index);
    const headerFill = timeline.unit === 'day' && column.isWeekend ? '#FFF7ED' : '#EEF3F9';
    const dateFill = timeline.unit === 'day' && column.isWeekend ? '#FFFBEB' : '#F5F8FC';
    const dateColor = timeline.unit === 'day' && column.isWeekend ? '#B45309' : '#64748B';
    const headText = timeline.unit === 'week' ? `W${column.index}` : column.label;
    const dateText = timeline.unit === 'week' ? column.label : column.subLabel;
    rect(x, padding + monthRowHeight, column.widthPx, weekRowHeight, headerFill);
    rect(x, padding + monthRowHeight + weekRowHeight, column.widthPx, dateRowHeight, dateFill);
    parts.push(`<text class="head" x="${x + column.widthPx / 2}" y="${padding + monthRowHeight + weekRowHeight / 2}" fill="#334155" text-anchor="middle">${svgText(headText)}</text>`);
    parts.push(`<text class="date" x="${x + column.widthPx / 2}" y="${padding + monthRowHeight + weekRowHeight + dateRowHeight / 2}" fill="${dateColor}" text-anchor="middle">${svgText(dateText)}</text>`);
  }
  ...
  for (const item of project.rows) {
    ...
    for (const column of timeline.columns) {
      const x = timelineLeft + columnLeftPx(column.index);
      const background = isPhase ? '#F3F7FF' : '#F9FBFE';
      const fillColor = timeline.unit === 'day' && column.isWeekend ? (isPhase ? '#FFF7ED' : '#FFFBEB') : background;
      rect(x, y, column.widthPx, rowHeight, fillColor, '#E3EAF4');
    }
    ...
    if (item.kind === 'sub' && item.milestone) {
      ...
      const markerGapPx = Number.isFinite(item.markerGapPx) ? item.markerGapPx : MILESTONE_BAR_TO_STAR_GAP_PX;
      const starX = milestonePoint + markerGapPx;
      ...
      parts.push(`<text class="milestone" x="${labelX}" y="${y + rowHeight / 2}" fill="#9F1239">${svgText(`${item.name} (${fmtMd(item.end)})`)}</text>`);
    }
