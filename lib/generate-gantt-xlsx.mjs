...
function normalizeSubtasks(task, taskName, taskStart, taskEnd) {
  ...
  return subtasks.map((subtask, subtaskIndex) => {
    ...
    return {
      ...
      ownerClient: String(subtask?.ownerClient || '').trim(),
      ownerVendor: String(subtask?.ownerVendor || '').trim(),
      priority: normalizeSubtaskPriority(subtask?.priority),
      deps: Array.isArray(subtask?.deps) ? subtask.deps.map((dep) => String(dep || '').trim()).filter(Boolean) : [],
    };
  });
}
