...
function estimateTextWidthPx(text) {
  ...
  for (const ch of String(text)) {
    ...
    if (ch === ' ') width += 4;
    else if (code <= 0x007f) width += /[A-Za-z0-9]/.test(ch) ? 7 : 4;
    else width += 13;
  }
  return Math.max(84, width + 18);
}
...
function getExtraWeeksForMilestones(projectStart, phases, baseWeeks) {
  ...
  phases.forEach((phase) => {
    phase.tasks.forEach((task) => {
      ...
      const exactWidthPx = Math.round((durationDays * WEEK_WIDTH_PX) / 7);
      const shellWidthPx = Math.max(12, exactWidthPx + 4);
      const markerRightPx = leftPx + shellWidthPx + 6 + 16 + 4 + estimateMilestoneLabelWidthPx(task);
...
function buildGanttSvg(project) {
  ...
  const weekRowHeight = 22;
  const dateRowHeight = 22;
  const headerHeight = monthRowHeight + weekRowHeight + dateRowHeight;
  const padding = 10;
  const timelineLeft = padding + leftA + leftB;
  const width = padding * 2 + leftA + leftB + project.totalWeeks * weekWidth + 24;
  ...
  const headCells = [
    { x: padding, w: leftA, text: '项目阶段', fill: '#DDE8FA', color: '#1E3A8A', align: 'middle' },
    { x: padding + leftA, w: leftB, text: '用时', fill: '#DFF2F8', color: '#0F4C63', align: 'middle' },
  ];
  for (const cell of headCells) {
    parts.push(`<rect x="${cell.x}" y="${headerY}" width="${cell.w}" height="${headerHeight}" fill="${cell.fill}" stroke="#C2D1E1"/>`);
    const textX = cell.align === 'left' ? cell.x + 12 : cell.x + cell.w / 2;
    const anchor = cell.align === 'left' ? 'start' : 'middle';
    parts.push(`<text class="head" x="${textX}" y="${headerY + Math.round(headerHeight / 2) + 5}" fill="${cell.color}" text-anchor="${anchor}">${cell.text}</text>`);
  }

  for (const band of monthBands) {
    const x = timelineLeft + (band.fromWeek - 1) * weekWidth;
    const bandWidth = (band.toWeek - band.fromWeek + 1) * weekWidth;
    parts.push(`<rect x="${x}" y="${headerY}" width="${bandWidth}" height="${monthRowHeight}" fill="#E6EBF3" stroke="#D4DDEA"/>`);
    parts.push(`<text class="head" x="${x + bandWidth / 2}" y="${headerY + 15}" fill="#334155" text-anchor="middle">${escapeXml(band.label)}</text>`);
  }

  for (const week of weekColumns) {
    const x = timelineLeft + (week.weekIndex - 1) * weekWidth;
    parts.push(`<rect x="${x}" y="${headerY + monthRowHeight}" width="${weekWidth}" height="${weekRowHeight}" fill="#EEF2F8" stroke="#D4DDEA"/>`);
    parts.push(`<rect x="${x}" y="${headerY + monthRowHeight + weekRowHeight}" width="${weekWidth}" height="${dateRowHeight}" fill="#F5F8FC" stroke="#D4DDEA"/>`);
    parts.push(`<text class="head" x="${x + weekWidth / 2}" y="${headerY + monthRowHeight + 15}" fill="#334155" text-anchor="middle">W${week.weekIndex}</text>`);
    parts.push(`<text class="small" x="${x + weekWidth / 2}" y="${headerY + monthRowHeight + weekRowHeight + 15}" fill="#64748B" text-anchor="middle">${fmtMd(week.start)}-${fmtMd(week.end)}</text>`);
  }
  ...
  for (let index = 0; index < project.rows.length; index += 1) {
    ...
    parts.push(`<rect x="${left}" y="${barY}" width="${barWidth}" height="${barHeight}" rx="${barHeight / 2}" fill="${colors.subSvg}" stroke="#DCE4F0"/>`);
